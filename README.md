...
Website for the slop-mop quality gate orchestration tool.
...
`deploy:app` follows the same worktree-safe pattern used in the sibling ScienceIsNeato sites: it builds the repo, allocates a local port, starts `wrangler dev` in a tracked background session, and records enough state to stop or inspect that deployment later.

## Structure

- `public/index.html` — landing page
- `scripts/deploy_app.sh` — tracked local deploy helper
- `wrangler.toml` — Cloudflare Worker static asset config
*** Add File: /Users/pacey/Documents/SourceCode/slop-mop-website/scripts/deploy_app.sh
#!/usr/bin/env bash
